...
<html>
    ...
    <body>
         <!-- poco phone section -->
    <div class="heading" id="samsung_heading">Poco Phone's</div>
    ...
    <?php
        ...
        // print_r($page);
        $pocopage--;
        $pocooffset = ($pocopage) * $pocolimitt;
        $pocosql = "SELECT * FROM `poco`  LIMIT $pocooffset ,$pocolimitt";
        ...
        ?>
            <div class="ele  border-gradient-green ">
                <form action="managecart.php?action=add&id=<?php echo $pocopd['s_no']; ?>" method="POST">
                    <div class="for_image">
        ...
        <?php
        $pocopagi = 'select * from `trending` ';
        $pocotoexecute = mysqli_query($conn, $pocopagi);
        if (mysqli_num_rows($pocotoexecute) > 0) {
            $pocotopassintotal = mysqli_num_rows($pocotoexecute);
            $pocolimitt = 15;
            $pocototalpage = ceil($pocotopassintotal / $pocolimitt);
            echo '<ul class="ultag">';
            for ($i = 1; $i <= $pocototalpage; $i++) {
                echo '<li class="litag"><a href="poco.php?page='.$i.'">' . $i . '</a></li>';
            }
            echo '</ul>';
        }
