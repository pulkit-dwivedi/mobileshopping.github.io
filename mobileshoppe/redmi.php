...
<html>
  ...
  top: 1%;
    ...
    <body>
         <!-- redmi phone section -->
    <div class="heading" id="redmi_heading">Redmi Phone's</div>
    ...
    <?php
        ...
        // print_r($page);
        $redmipage--;
        $redmioffset = ($redmipage) * $redmilimitt;
        $redmisql = "SELECT * FROM `redmi`  LIMIT $redmioffset ,$redmilimitt";
        ...
        ?>
            <div class="ele  border-gradient-green ">
                <form action="managecart.php?action=add&id=<?php echo $redmipd['s_no']; ?>" method="POST">
                    <div class="for_image">
        ...
        <?php
        $redmipagi = 'select * from `redmi` ';
        $redmitoexecute = mysqli_query($conn, $redmipagi);
        if (mysqli_num_rows($redmitoexecute) > 0) {
            $redmitopassintotal = mysqli_num_rows($redmitoexecute);
            $redmilimitt = 16;
            $redmitotalpage = ceil($redmitopassintotal / $redmilimitt);
            echo '<ul class="ultag">';
            for ($i = 1; $i <= $redmitotalpage; $i++) {
                echo '<li class="litag"><a href="redmi.php?page='.$i.'">' . $i . '</a></li>';
            }
            echo '</ul>';
        }
